...
namespace XboxLive.MACS.ASN
{
    public static class AsnIO
    {
            ...
            bool strictDER, out string pemType)
        {
             ...
             * return it.
             */
            if (LooksLikeBER(buf, strictDER)) return buf;
             ...
             * occur in between.
             */
            var p = str.IndexOf("-----BEGIN ");
            var q = str.IndexOf("-----END ");
            if (p >= 0 && q >= 0)
            {
                p += 11;
                var r = str.IndexOf((char) 10, p) + 1;
                var px = str.IndexOf('-', p);
        ...
        {
            var orig = off;
            if (!DecodeTag(buf, lim, ref off)) return -1;
            var len = DecodeLength(buf, lim, ref off);
            if (len >= 0)
            {
                if (len > lim - off) return -1;
                return off + len - orig;
            }
            ...
            for (;;)
            {
                var slen = BERLength(buf, lim, off);
                if (slen < 0) return -1;
                off += slen;
                if (slen == 2 && buf[off] == 0) return off - orig;
            }
        }
        ...
        {
            var lim = off + len;
            var objLen = BERLength(buf, lim, off);
            if (objLen != len) return false;
            if (strictDER)
            {
